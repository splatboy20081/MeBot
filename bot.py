...
async def status_task():
    await client.change_presence(activity=discord.Game("Minecraft and s!help"))
...
async def info(context):
    if context.message.author.id in BLACKLIST:
        ...
        e = discord.Embed(description='Used splatboy20081\'s bot', color=0x00FF00)
        e.set_author(name="Bot Informations")
        e.add_field(name="Owner:", value="splatboy20081#6989", inline=True)
...
async def invite(context):
	if context.message.author.id in BLACKLIST:
		...
		await context.message.channel.send('Invite me by clicking here: https://discordapp.com/oauth2/authorize?&client_id=700385092803100712&scope=bot&permissions=8')
...
async def eight_ball(context, *args):
	if context.message.author.id in BLACKLIST:
		...
		answers = ['It is certain.', 'It is decidedly so.', 'You may rely on it.', 'Without a doubt.',
				   ...
				   'My sources say no.', 'Outlook not so good.', 'Very doubtful.', 'Idk about that...' 'Tuesday']
	...
	if context.message.author.id in BLACKLIST:
		...
		if context.message.author.guild_permissions.kick_members:
			if member.guild_permissions.administrator:
				...
				await context.message.channel.send(embed=embed)
				await context.message.delete()
				await member.send('You are warned by **{0}**!  '.format(context.message.author) + 'Reason for warn: {0}'.format(mesg))
				await member.kick()
		else:
			embed = discord.Embed(title='Error!', description='You don\'t have the permission to use this command.',
								  color=0x00FF00)
	...
	if context.message.author.id in BLACKLIST:
		...
		if context.message.author.guild_permissions.administrator:
			...
			embed = discord.Embed(title='User Warned!',
								  description='**{0}** has been warned by **{1}**!'.format(member, context.message.author),
								  color=0x00FF00)
			embed.add_field(name='Reason:', value=mesg)
			await context.message.channel.send(embed=embed)
			await context.message.delete()
			await member.send('You are warned by **{0}**!  '.format(
				context.message.author) + 'Reason: {0}'.format(mesg))
		else:
			embed = discord.Embed(title='Error!', description='You don\'t have the permission to use this command.',
								  color=0x00FF00)
